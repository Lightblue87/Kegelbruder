#!/usr/bin/env python3
"""
Kegel Brüder App - BUGFIX VERSION

Spieler- & Kassenverwaltung inkl. Anwesenheit, Abrechnung, Kassenbuch

FIXES ENTHALTEN:
1. Race Condition beim Rollback
2. Division by Zero beim Stechen
3. KeyError bei Zusatzfeldern
4. Dateninkonsistenz beim Spielstart
5. Fehler bei leerer Spielerliste
6. KeyError bei Gast-Spielern
7. Atomare Dateischreibvorgänge
"""
...
# -------------------- Dateien --------------------
DATA_FILE = "kegel_brueder.json"
DATA_FILE_KASSE = "kasse.json"
AKTUELLES_SPIEL = "aktuelles_spiel.json"
HISTORIE_FILE = "historie.json"
MITGLIEDER_DATEI = "mitglieder.json"
...
class AtomicFileWriter:
    ...
    def atomic_write(file_path, data, backup=True):
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(mode='w', delete=False, suffix='.json', dir=os.path.dirname(file_path) or '.') as f:
...
class KegelBruederApp:
    ...
    def abort_game(self, silent: bool = False):
        ...
        - Rollback aller in dieser Session gebuchten Ein-/Auszahlungen
        - EXAKTE Wiederherstellung der offenen Beträge
        - Keine Archivierung
        - UI zurücksetzen
        """
        if not silent:
        ...
        # ========= FIX #1: Race Condition beim Rollback =========
        try:
            for kind, betrag, expected_text in reversed(getattr(self, "_session_tx", [])):
                # Guard: Wenn Transaktionen leer, komplett abbrechen
                ...
                if last != expected_text:
                    # Bei Unstimmigkeit komplett stoppen (nicht weitermachen)
                    logging.warning(f"Transaktions-Mismatch: {last} != {expected_text}. Rollback stoppt.")
                    break  # ← FIX: break statt continue
        ...
        # ========= FIX #5: Guard gegen leere Spielerliste =========
        try:
    ...
    def create_punkteingabe(self):
        ...
        for player in self.player_order:
            ...
            self.zusatzfelder[player] = {}  # ========= FIX #6: IMMER initialisieren =========
    ...
    def update_kassenstand(self):
        """Aktualisiert den Kassenstand in der GUI"""
        try:
    ...
    def _session_einzahlung(self, betrag: float, beschreibung: str, spieler=None):
        """Einzahlung buchen UND in der Session protokollieren"""
        if betrag <= 0:
            return
    ...
    def _session_auszahlung(self, betrag: float, beschreibung: str):
        """Auszahlung buchen UND in der Session protokollieren"""
        if betrag <= 0:
            return
    ...
    def show_anwesenheit(self):
        ...
        self.anwesenheit_vars = {}
        self.bezahlung_vars  = {}
        self.gastspieler_vars = []
    ...
    def start_game(self, window):
        ...
        # ========= FIX #4: Dateninkonsistenz beim Spielstart =========
        mit = DatenHandler.laden_mitglieder().get("players", {})
        self.players = {}
        for p in aktive:
            original = mit.get(p, {})  # ← Aus Quelle laden
            self.players[p] = {
                "typ": original.get("typ", "Stamm"),  # ← Korrekt aus Mitgliedern
                "punkte": [0, 0, 0, 0],
                "offene_zahlung": float(original.get("offene_zahlung", 0.0)),  # ← Korrekt
    ...
    def berechne_abrechnung(self):
        """Liefert pro Spieler den zu zahlenden Betrag"""
        ...
        counts = {}
        for p in self.players:
            # ========= FIX #3: KeyError bei Zusatzfeldern =========
            if p not in self.zusatzfelder:  # ← Guard
    ...
    def berechne_strafen_anteile(self):
        """Liefert pro Spieler den Betrag für Strafen"""
    ...
    def abrechnung_speichern(self):
        ...
        if info_summe > 0:
            self.kasse.einzahlung(info_summe, f"{datum} - Einnahmen vom Spieltag")
    ...
    def sperre_spielfelder(self, zeige_popup: bool = True):
        """Sperrt alle Spielfelder nach der Abrechnung"""
    ...
    def run_stechen(self, teilnehmer, art="punkte"):
        ...
        while remaining:
            # ========= FIX #2: Division by Zero beim Stechen =========
            if not remaining:  # ← Guard
                break
            ...
            if not remaining:  # ← Guard vor max()
                break
    ...
    def resolve_points_tiebreak(self):
        ...
        pump_counts  = {p: self.zusatzfelder[p]["Pumpen"].get() if p in self.zusatzfelder else 0 for p in self.players}

        pump_rank = {}
        if pump_counts:
            # ========= FIX #2: Division by Zero =========
            pump_values = list(pump_counts.values())
            if pump_values:  # ← Guard
    ...
    def _snapshot_has_content(self, snap_players: dict) -> bool:
        """True, wenn im Snapshot überhaupt Eingaben vorliegen"""
    ...
    def save_runtime_snapshot(self):
        """Speichert den aktuellen UI-Stand"""
        try:
            snap_players = {}
            for player, entry_list in self.punkte_entries.items():
                ...
                # ========= FIX #3: KeyError bei Zusatzfeldern =========
                if player not in self.zusatzfelder:  # ← Guard
                    pu = nn = kr = 0
                else:
    ...
    def load_runtime_snapshot(self):
        """Lädt AKTUELLES_SPIEL und befüllt die GUI"""
        try:
    ...
    def _build_tab_order(self):
        """Tab-Reihenfolge vertikal pro Runde"""
